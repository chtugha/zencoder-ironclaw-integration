...
1. If a `task_id` from a prior `zencoder-tool` response is present in this conversation: use `get_plan`, `update_plan_step`, `add_plan_steps`, and `create_plan` as the canonical plan store. Do NOT create a parallel local memory-doc plan (`memory_write` to `plans/<slug>.md`). `get_plan` requires `project_id` + `task_id`; if `project_id` is absent, call `list_projects` first.
2. If no `task_id` is in scope: fall through to native plan-mode behavior (memory-doc plans, `plan_update`, `mission_create`/`mission_fire`).

---

<!-- upstream: plan-mode v0.1.0 nearai/ironclaw/skills/plan-mode/SKILL.md @main -->
